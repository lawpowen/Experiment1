...
void check_priority_to_be_donated()
{
  ...
  {
    struct thread *t = list_entry(list_front(&thread_current()->threads_want_lock), struct thread, twl_elem);
...
void 
mlfqs_increase_recent_cpu_by_1 (void)
{
	ASSERT (thread_mlfqs);
	ASSERT (intr_context ());
